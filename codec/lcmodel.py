...
class LCEncoder(torch.nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, layers, class_num, sememe_num, lexname_num, rootaffix_num):
        ...
        self.aggregate =  torch.nn.Conv1d(in_channels=4, out_channels=1, kernel_size=1)
    ...
    def forward(self, operation, x=None, w=None, ws=None, wl=None, wr=None, msk_s=None, msk_l=None, msk_r=None, mode=None):
        ...
        ########## learning channels ##########
        # print ("running lc!")
        all_scores = torch.stack([score0, score_s, score_r, score_l], dim = 1)
        score = torch.squeeze(self.aggregate(all_scores))
        ########## learning channels ##########
