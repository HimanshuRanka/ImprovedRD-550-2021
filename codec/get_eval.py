...
def main(frequency, batch_size, epoch_num, verbose, MODE, lc, directory, loss, args):
    mode = MODE
    if lc: 
        Encoder = LCEncoder
    else: 
    ...
    model = Encoder(vocab_size=len(word2index), embed_dim=word2vec.shape[1], hidden_dim=300, layers=1, class_num=label_size, sememe_num=sememe_num, lexname_num=lexname_num, rootaffix_num=rootaffix_num, loss=loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dr', '--save_dir', type=str, default='../runs/default/')
    parser.add_argument('-ls', '--loss', type=str, default='ce')

    parser.add_argument('-pt', '--path', type=str, default='../runs/default/')

    args = parser.parse_args()
    setup_seed(args.seed)
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    main(args.frequency, args.batch_size, args.epoch_num, args.verbose, args.mode, 
         args.learn_channel, args.save_dir, args.loss, args)